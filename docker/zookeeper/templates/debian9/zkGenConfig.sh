...
#
# The zkGenConfig.sh script will generate the ZooKeeper configuration (zoo.cfg),
# Log4J configuration (log4j.properties), and JVM configuration (jvm.env).
# These will be written to the /opt/zookeeper/conf directory with correct
# read permissions for the zookeeper user. These files are generated from
# environment variables that are injected into the container as in the example,
...
function validate_env() {
    ...
    if [ -z $ZK_REPLICAS ]; then
        echo "ZK_REPLICAS is a mandatory environment variable for Quorum"
        exit 1
    fi

...
helm template "${APP_INSTANCE_NAME}" chart/harbor \
  ...
  --set core.image.repository="${IMAGE_HARBOR_CORE}" \
  --set core.image.tag="${TAG}" \
  --set chartmuseum.image.repository="${IMAGE_HARBOR_CHARTMUSEUM}" \
  --set chartmuseum.image.tag="${TAG}" \
  --set database.internal.image.repository="${IMAGE_HARBOR_DATABASE}" \
  --set database.internal.image.tag="${TAG}" \
  --set jobservice.image.repository="${IMAGE_HARBOR_JOBSERVICE}" \
  --set jobservice.image.tag="${TAG}" \
  --set notary.server.image.repository="${IMAGE_HARBOR_NOTARY_SERVER}" \
  --set notary.server.image.tag="${TAG}" \
  --set notary.signer.image.repository="${IMAGE_HARBOR_NOTARY_SIGNER}" \
  --set notary.signer.image.tag="${TAG}" \
  --set portal.image.repository="${IMAGE_HARBOR_PORTAL}" \
  --set portal.image.tag="${TAG}" \
  --set redis.internal.image.repository="${IMAGE_HARBOR_REDIS}" \
  --set redis.internal.image.tag="${TAG}" \
  --set registry.registry.image.repository="${IMAGE_HARBOR_REGISTRY}" \
  --set registry.registry.image.tag="${TAG}" \
  --set registry.controller.image.repository="${IMAGE_HARBOR_REGISTRYCTL}" \
  --set registry.controller.image.tag="${TAG}" \
  --set trivy.image.repository="${IMAGE_HARBOR_TRIVY}" \
  --set trivy.image.tag="${TAG}" \
  --set nginx.image.repository="${IMAGE_HARBOR_NGINX}" \

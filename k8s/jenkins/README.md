...
# Deploying slaves in kubernetes
...
kubectl get secret $(kubectl get sa jenkins -n $NAMESPACE -o jsonpath="{.secrets[0].name}") -n jenkins -o jsonpath="{.data.token}" | base64 --decode
```
...
```
# CA Certificate
kubectl get secret $(kubectl get sa jenkins -n $NAMESPACE -o jsonpath="{.secrets[0].name}") -n jenkins -o jsonpath={.data.'ca\.crt'} | base64 --decode
```
...
```
echo jenkins-1-jenkins-agents-connector.$NAMESPACE.svc.cluster.local:50000
```
...
You can use the follow pipeline example to build a Golang project and docker image without any further configuration using [Kaniko](https://github.com/GoogleContainerTools/kaniko):
```
pipeline {
agent {
        kubernetes {
